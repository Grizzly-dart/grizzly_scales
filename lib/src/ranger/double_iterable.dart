...
class DoubleRangeIterable extends IterableBase<double> {
	...
	factory DoubleRangeIterable.until(double stop, [double step = 1.0]) {
		if (step == 0.0) throw new ArgumentError.value(step, 'step', 'Must not be 0');
	...
	factory DoubleRangeIterable.linspace(double start, double stop, [int count = 50]) {
		if (count == 0) throw new ArgumentError.value(count, 'count', 'Must not be 0');
	...
	bool operator ==(other) => other is DoubleRangeIterable &&
			start == other.start &&
			stop == other.stop &&
			step == other.step;
}
